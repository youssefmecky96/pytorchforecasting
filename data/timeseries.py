...
class TimeSeriesDataSet(Dataset):
    ...
    * etc.
    ...
    def __init__(
        ...
        target_normalizer: Union[NORMALIZER, str,
                                 List[NORMALIZER], Tuple[NORMALIZER]] = "auto",
        categorical_encoders: Dict[str, NaNLabelEncoder] = {},
        scalers: Dict[str, Union[StandardScaler, RobustScaler,
        ...
        assert isinstance(self.max_encoder_length,
                          int), "max encoder length must be integer"
        ...
        assert isinstance(self.min_encoder_length,
                          int), "min encoder length must be integer"
        self.max_prediction_length = max_prediction_length
        assert isinstance(self.max_prediction_length,
                          int), "max prediction length must be integer"
        ...
        assert isinstance(self.min_prediction_length,
                          int), "min prediction length must be integer"
        ...
        self.constant_fill_strategy = {} if len(
            constant_fill_strategy) == 0 else constant_fill_strategy
        self.predict_mode = predict_mode
        self.allow_missing_timesteps = allow_missing_timesteps
        self.target_normalizer = target_normalizer
        self.categorical_encoders = {} if len(
            categorical_encoders) == 0 else categorical_encoders
        self.scalers = {} if len(scalers) == 0 else scalers
        self.add_target_scales = add_target_scales
        self.variable_groups = {} if len(
            variable_groups) == 0 else variable_groups
        ...
        if self.add_relative_time_idx:
            ...
            # dummy - real value will be set dynamiclly in __getitem__()
            data.loc[:, "relative_time_idx"] = 0.0
                ...
                if name in self.time_varying_known_reals:
                    ...
                    for lagged_name in lagged_names:
                        if lagged_name not in self.time_varying_known_categoricals:
                            self.time_varying_known_categoricals.append(
                                lagged_name)
                    ...
                    for lagged_name, lag in lagged_names.items():
                        if lag < self.max_prediction_length:  # keep in unknown as if lag is too small
                            if lagged_name not in self.time_varying_unknown_reals:
                                self.time_varying_unknown_reals.append(
                                    lagged_name)
                        else:
                            if lagged_name not in self.time_varying_known_reals:
                                # switch to known so that lag can be used in decoder directly
                                self.time_varying_known_reals.append(
                                    lagged_name)
        ...
        if min_prediction_idx is not None:
            ...
            data = data[lambda x: x[self.time_idx] >=
        ...
        self.index = self._construct_index(
            data, predict_mode=self.predict_mode)
    ...
    def lagged_targets(self) -> Dict[str, str]:
        ...
        vars = {}
        for name in self.lags:
            vars.update({lag_name: name for lag_name in self._get_lagged_names(
                name) if name in self.target_names})
        return vars
            ...
            for target in self.target_names:
                if data[target].dtype.kind != "f":  # category
                    ...
                    if self.add_target_scales:
                        warnings.warn(
                            "Target scales will be only added for continous targets", UserWarning)
                else:
                    ...
                    if self.max_encoder_length > 20 and self.min_encoder_length > 1:
                        normalizers.append(EncoderNormalizer(
                            transformation=transformer))
                    else:
                        normalizers.append(GroupNormalizer(
                            transformation=transformer))
    ...
    def _validate_data(self, data: pd.DataFrame):
        ...
        for name in self.flat_categoricals:
            if name not in data.columns:
                raise KeyError(
                    f"variable {name} specified but not found in data")
            if not (
    ...
    def _preprocess_data(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for name, group_name in self._group_ids_mapping.items():
            ...
            encoder = deepcopy(self.categorical_encoders.get(
                group_name, NaNLabelEncoder()))
        ...
        if self.target_normalizer is not None:
            ...
            if isinstance(self.target_normalizer, EncoderNormalizer):
                ...
                copy_kwargs = {name: getattr(
                    self.target_normalizer, name) for name in common_init_args}
                normalizer = GroupNormalizer(
                    groups=self.group_ids, **copy_kwargs)
                ...
                data[self.target] = self.target_normalizer.transform(
                    data[self.target])
                # overwrite target because it requires encoding (continuous targets should not be normalized)
            ...
            if name in self.target_names or name in self.lagged_variables or len(self.scalers) == 0:
                ...
                try:
                    ...
                    if isinstance(self.scalers[name], GroupNormalizer):
                        self.scalers[name] = self.scalers[name].fit(
                            data[name], data)
                    else:
                        self.scalers[name] = self.scalers[name].fit(
                            data[[name]])
    ...
    def get_transformer(self, name: str, group_id: bool = False):
        ...
        if name in self.flat_categoricals + self.group_ids + self._group_ids:
            name = self.variable_to_group_mapping.get(
                name, name)  # map name to encoder

            # take target normalizer if required
            if name in self.target_names:
                transformer = self.target_normalizers[self.target_names.index(
                    name)]
            else:
    ...
    def _data_to_tensors(self, data: pd.DataFrame) -> Dict[str, torch.Tensor]:
        ...
        index = check_for_nonfinite(
            torch.tensor(data[self._group_ids].to_numpy(
                np.int64), dtype=torch.int64), self.group_ids
        )
        time = check_for_nonfinite(
            torch.tensor(data["__time_idx__"].to_numpy(
                np.int64), dtype=torch.int64), self.time_idx
        )

        # categorical covariates
        categorical = check_for_nonfinite(
            torch.tensor(data[self.flat_categoricals].to_numpy(
                np.int64), dtype=torch.int64), self.flat_categoricals
        )
        ...
        if isinstance(self.target_normalizer, NaNLabelEncoder):
            target = [
                check_for_nonfinite(
                    torch.tensor(data[f"__target__{self.target}"].to_numpy(
                        dtype=np.int64), dtype=torch.long),
                    self.target,
                )
            ]
        else:
                    ...
                    check_for_nonfinite(
                        torch.tensor(
                            data[f"__target__{name}"].to_numpy(
                                dtype=[np.float64,
                                       np.int64][data[name].dtype.kind in "bi"]
                            ),
                            dtype=[torch.float,
                                   torch.long][data[name].dtype.kind in "bi"],
                        ),
                        name,
                    )
        ...
        continuous = check_for_nonfinite(
            torch.tensor(data[self.reals].to_numpy(
                dtype=np.float64), dtype=torch.float), self.reals
        )
    ...
    def _construct_index(self, data: pd.DataFrame, predict_mode: bool) -> pd.DataFrame:
        ...
        df_index_first = g["__time_idx__"].transform(
            "first").to_frame("time_first")
        df_index_last = g["__time_idx__"].transform(
            "last").to_frame("time_last")
        ...
        df_index["count"] = (df_index["time_last"] -
                             df_index["time_first"]).astype(int) + 1
        ...
        max_time = (df_index["time"] + max_sequence_length -
                    1).clip(upper=df_index["count"] + df_index.time_first - 1)
        ...
        df_index["sequence_length"] = df_index["time"].iloc[df_index["index_end"]
    ...
    def filter(self, filter_func: Callable, copy: bool = True) -> "TimeSeriesDataSet":
        ...
        """
        # calculate filter
        filtered_index = self.index[np.asarray(
            filter_func(self.decoded_index))]
        # raise error if filter removes all entries
        if len(filtered_index) == 0:
            raise ValueError(
                "After applying filter no sub-sequences left in dataset")
        if copy:
            dataset = _copy(self)
    ...
    def decoded_index(self) -> pd.DataFrame:
        ...
        index = (
            ...
            pd.DataFrame(self.data["groups"]
                         [index_start].numpy(), columns=self.group_ids)
            # to original values
    ...
    def set_overwrite_values(
        ...
        """
        values = torch.tensor(self.transform_values(
            variable, np.asarray(values).reshape(-1), inverse=False)).squeeze()
        assert target in [
            "all",
            "decoder",
            "encoder",
    ...
    def __getitem__(self, idx: int) -> Tuple[Dict[str, torch.Tensor], torch.Tensor]:
        ...
        target = [d[index.index_start: index.index_end + 1].clone()
        ...
        if sequence_length < index.sequence_length:
            ...
            indices = torch.repeat_interleave(
                torch.arange(len(time)), repetitions)
            ...
            for name, value in self.encoded_constant_fill_strategy.items():
                if name in self.reals:
                    data_cont[repetition_indices,
                              self.reals.index(name)] = value
                elif name in [f"__target__{target_name}" for target_name in self.target_names]:
                    target_pos = self.target_names.index(
                        name[len("__target__"):])
                    target[target_pos][repetition_indices] = value
                elif name in self.flat_categoricals:
                    data_cat[repetition_indices,
                             self.flat_categoricals.index(name)] = value
        ...
        if self.randomize_length is not None:  # randomization improves generalization
            ...
            encoder_length_probability = Beta(
                self.randomize_length[0], self.randomize_length[1]).sample()
            ...
            new_decoder_length = min(
                decoder_length + (encoder_length - new_encoder_length), self.max_prediction_length)

            # select subset of sequence of new sequence
            if new_encoder_length + new_decoder_length < len(target[0]):
                data_cat = data_cat[encoder_length -
                                    new_encoder_length: encoder_length + new_decoder_length]
                data_cont = data_cont[encoder_length -
                                      new_encoder_length: encoder_length + new_decoder_length]
                target = [t[encoder_length - new_encoder_length: encoder_length +
                            new_decoder_length] for t in target]
        ...
        if self.add_relative_time_idx:
            data_cont[:, self.reals.index("relative_time_idx")] = (
                torch.arange(-encoder_length, decoder_length,
                             dtype=data_cont.dtype) / self.max_encoder_length
            )

        if self.add_encoder_length:
            data_cont[:, self.reals.index("encoder_length")] = (
                (encoder_length - 0.5 * self.max_encoder_length) /
                self.max_encoder_length * 2.0
            )
        ...
        for idx, target_normalizer in enumerate(self.target_normalizers):
            if isinstance(target_normalizer, EncoderNormalizer):
                ...
                if target_name in self.reals:
                    data_cont[:, self.reals.index(
                        target_name)] = target_normalizer.transform(target[idx])
                if self.add_target_scales:
        ...
        if self._overwrite_values is not None:
            ...
            if self._overwrite_values["variable"] in self.reals:
                ...
                idx = self.flat_categoricals.index(
                    self._overwrite_values["variable"])
    ...
    def _collate_fn(
        ...
        decoder_time_idx_start = (
            torch.tensor([batch[0]["encoder_time_idx_start"]
                         for batch in batches], dtype=torch.long) + encoder_lengths
        )
        decoder_time_idx = decoder_time_idx_start.unsqueeze(
            1) + torch.arange(decoder_lengths.max()).unsqueeze(0)
            ...
            for idx in range(len(batches[0][0]["target_scale"])):
                if isinstance(batches[0][0]["target_scale"][idx], torch.Tensor):  # stack tensor
                    ...
                    scale = torch.from_numpy(
                        np.array([batch[0]["target_scale"][idx]
                                 for batch in batches], dtype=np.float32),
                    )
                target_scale.append(scale)
        else:  # convert to tensor
            target_scale = torch.from_numpy(
                np.array([batch[0]["target_scale"]
                         for batch in batches], dtype=np.float32),
            )
        ...
        if isinstance(batches[0][1][0], (tuple, list)):
            target = [
                rnn.pad_sequence([batch[1][0][idx]
                                 for batch in batches], batch_first=True)
                for idx in range(len(batches[0][1][0]))
            ]
            encoder_target = [
                rnn.pad_sequence([batch[0]["encoder_target"][idx]
                                 for batch in batches], batch_first=True)
                for idx in range(len(batches[0][1][0]))
            ]
        else:
            target = rnn.pad_sequence([batch[1][0]
                                      for batch in batches], batch_first=True)
            encoder_target = rnn.pad_sequence(
                [batch[0]["encoder_target"] for batch in batches], batch_first=True)

        if batches[0][1][1] is not None:
            weight = rnn.pad_sequence([batch[1][1]
                                      for batch in batches], batch_first=True)
        else:
            weight = None
