...
class TemporalFusionTransformer(BaseModelWithCovariates):
    def __init__(
        ...
        * non-linear variable processing in variable selection network can be shared among decoder and encoder
        ...
        assert isinstance(
            loss, LightningMetric), "Loss has to be a PyTorch Lightning `Metric`"
        ...
        self.prescalers = nn.ModuleDict(
            {
                name: nn.Linear(1, self.hparams.hidden_continuous_sizes.get(
    ...
    def get_attention_mask(self, encoder_lengths: torch.LongTensor, decoder_lengths: torch.LongTensor):
        ...
        if self.hparams.causal_attention:
            ...
            predict_step = torch.arange(
                0, decoder_length, device=self.device)[:, None]
            # do not attend to steps to self or after prediction

            decoder_mask = (attend_step >= predict_step).unsqueeze(
                0).expand(encoder_lengths.size(0), -1, -1)
        else:
            ...
            decoder_mask = create_mask(decoder_length, decoder_lengths).unsqueeze(
                1).expand(-1, decoder_length, -1)

        # do not attend to steps where data is padded
        encoder_mask = create_mask(encoder_lengths.max(), encoder_lengths).unsqueeze(
            1).expand(-1, decoder_length, -1)
    ...
    def forward(self, x: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        input_cell = self.static_context_initial_cell_lstm(
            static_embedding).expand(self.hparams.lstm_layers, -1, -1)

        # run local encoder
        encoder_output, (hidden, cell) = self.lstm_encoder(
            embeddings_varying_encoder, (input_hidden,
                                         input_cell), lengths=encoder_lengths, enforce_sorted=False
        )
        ...
        lstm_output = torch.cat(
            [lstm_output_encoder, lstm_output_decoder], dim=1)

        # static enrichment
        static_context_enrichment = self.static_context_enrichment(
            static_embedding)
        attn_input = self.static_enrichment(
            lstm_output, self.expand_static_context(
                static_context_enrichment, timesteps)
        )
    ...
    def interpret_output(
        ...
        if isinstance(out["decoder_attention"], (list, tuple)):
            ...
            max_last_dimension = max(x.size(-1)
                                     for x in out["decoder_attention"])
        ...
        if isinstance(out["encoder_attention"], (list, tuple)):
            ...
            encoder_attention = torch.gather(
                encoder_attention, dim=3, index=new_index)
        ...
        attention = torch.concat(
            [encoder_attention, decoder_attention], dim=-1)
        ...
        encode_mask = create_mask(
            encoder_variables.size(1), out["encoder_lengths"])
        ...
        decode_mask = create_mask(
            decoder_variables.size(1), out["decoder_lengths"])
        ...
        if reduction != "none":  # if to average over batches
            ...
            attention = attention / \
                masked_op(attention, dim=1,
                          op="sum").unsqueeze(-1)  # renormalize
        ...
        if plot_attention:
            ...
            for f in to_list(fig):
                ...
                ax2.plot(
                    ...
                    interpretation["attention"][0, -
                                                encoder_length:].detach().cpu(),
                    alpha=0.2,
                    color="k",
                )
                f.tight_layout()
    ...
    def plot_interpretation(self, interpretation: Dict[str, torch.Tensor]) -> Dict[str, plt.Figure]:
        ...
        * Variable selection weights / importances
        ...
        ax.plot(
            np.arange(-self.hparams.max_encoder_length, attention.size(0) -
                      self.hparams.max_encoder_length), attention
        )
        ...
        def make_selection_plot(title, values, labels):
            ...
            ax.barh(np.arange(len(values)),
                    values[order] * 100, tick_label=np.asarray(labels)[order])
            ax.set_title(title)
        ...
        figs["static_variables"] = make_selection_plot(
            "Static variables importance", interpretation["static_variables"].detach(
            ).cpu(), self.static_variables
        )
        figs["encoder_variables"] = make_selection_plot(
            "Encoder variables importance", interpretation["encoder_variables"].detach(
            ).cpu(), self.encoder_variables
        )
        figs["decoder_variables"] = make_selection_plot(
            "Decoder variables importance", interpretation["decoder_variables"].detach(
            ).cpu(), self.decoder_variables
        )
    ...
    def log_interpretation(self, outputs):
        ...
        interpretation = {
            ...
            name: padded_stack([x["interpretation"][name].detach()
                               for x in outputs], side="right", value=0).sum(0)
        ...
        attention_occurances = interpretation["encoder_length_histogram"][1:].flip(
            0).float().cumsum(0)
        ...
        attention_occurances = torch.cat(
            [
                ...
                torch.ones(
                    interpretation["attention"].size(
                        0) - attention_occurances.size(0),
        ...
        interpretation["attention"] = interpretation["attention"] / \
            attention_occurances.pow(2).clamp(1.0)
        interpretation["attention"] = interpretation["attention"] / \
            interpretation["attention"].sum()

        # make interpretation figures
        ...
        for type in ["encoder", "decoder"]:
            ...
            ax.set_title(
                f"{type.capitalize()} length distribution in {label} epoch")
